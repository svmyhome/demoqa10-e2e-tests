...
from demoqa10_e2e_tests.utils.step_logging import request_put_step_logging, request_patch_step_logging, response_logging
...
def test_update_user_put(base_url):
    ...
    response: Response = response_logging(f'{base_url}users/2', method='PUT', json=payload)
...
def test_update_user_patch(base_url):
    ...
    response: Response = response_logging(f'{base_url}users/2', method='PATCH', json=payload_patch)
